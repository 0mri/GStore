<template>
  <div class="form-container">
    ...
    <form
      ...
      id="my-sample-form"
      ref="checkOutForm"
      v-bind:class="`scale-down ${cardType ? cardType : ''}`"
    >
      <vue-simple-spinner v-if="isLoading" />
      <div v-show="!isLoading">
        <div class="cardinfo-card-number">
          <label class="cardinfo-label" for="card-number">Card Number</label>
          <div class="input-wrapper" id="card-number"></div>
          <div ref="cardImage" id="card-image" :class="cardType"></div>
        </div>
      ...
      <span class="tag is-danger" v-if="error">{{ error }}</span>
    </form>
    <b-button
      ...
      ref="buttonPay"
      type="is-secondary"
      v-bind:class="formValid ? 'show-button' : ''"
      native-type="submit"
      >Continue</b-button
    >
  </div>
...
export default {
  ...
  mounted() {
    if (!this.client_token) return
    //HOSTEDFIELDS
    var form = this.$refs.checkOutForm
    var submit = this.$refs.buttonPay.$el
        ...
        braintree.hostedFields.create(
          ...
          (err, hostedFieldsInstance) => {
            ...
            hostedFieldsInstance.on('validityChange', (event) => {
              ...
              this.formValid = Object.keys(event.fields).every((key) => {
